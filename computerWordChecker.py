                ...
                if spot not in occupied:
                    ...
                    while spaceCount != length:
                        if column != 15:
                            ...
                            spaceCount = length     # exits when number of letters is NOT satisfied
                    if len(spaceCombo) == length and CRC.conformsBetterRules(spaceCombo, attachments):
                                #checks if the combo attaches to the current board
                        rows.append(spaceCombo)     # does not append in the exit case
